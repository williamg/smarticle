...
function handleArticles (keyword, callback) {
	...
	search =	'https://user:' + key + '@api.datamarket.azure.com' + 
			'/Bing/Search/v1/Composite?Sources=%27news%27&Query=' + 
			'%27' + keyword  + '%27&Market=%27es-US%27&' +
			'$format=JSON';
...
function parseArticle (url, keyword, callback) {
	...
	request (readbilityRequest, function (err, res, body) { 
		...
		body.title = body.title.decodeHTMLEntities ();
...
function saveArticle (article) {
	...
	async.series ([
		...
		function (callback) {
			var artKeyQuery =	'INSERT INTO keyArt ' + 
						'(keyID, artID) VALUES ' +
						'("' + keyID + '", "' +
						artID + '")';
		...
		function (callback) {
			...
			for (var i = 0; i < words.length; i++) {
				var term = db.escape (words[i].term);
				var wordQuery =	'INSERT INTO words (word, ' +
						'artID, count) VALUES ("' +
						term + '", "' + artID + 
						'", "' + words[i].tf + '")';

				db.queryDB (wordQuery, function (err, rows) {
