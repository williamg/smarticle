...
exports.extract = function (text) {
	...
	_.each (options.ngrams, function(ngram) {
		...
		tokenized = _.map (natural.NGrams.ngrams (tokenized, ngram),
			function (ngram) {
				return ngram.join (' ').toLowerCase ();
		});	

		// Sort the keywords by frequency and eliminate blacklisted
		// words		
...
exports.combine = function (phrases, cutoff) {
	...
	_.each (_.keys (phrases), function (phrase) {
		...
		_.each (natural.NGrams.ngrams (phrase, ngramToTry), 
			function (ngram) {
